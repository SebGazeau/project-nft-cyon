...
contract Master is Auction {
    ...
    function createNFT(
        ...
        require(
            ...
            "The collection address needs to be different from zero."
        );
        require(
            _firstOwner != address(0),
            "The user address needs to be different from zero."
        );

        return (
            NFTCollections(_collectionAddress).MintNFT(
                _firstOwner,
                msg.sender,
                _tokenURI,
                _name,
    ...
    function setNewPrice(
        ...
        require(
            _collectionAddress != address(0),
            "The collection address needs to be different from zero."
        ); // Make sure the address is different from zero
        ...
        require(
            NFTCollections(_collectionAddress).ownerOf(_tokenID) == msg.sender,
            "This NFT does not belong to the current message sender."
        ); // Make sure the sale comes from the owner. Revert if the tokenID does not exist (ERC721Upgradeable library)
        ...
        payable
    {
        ...
        require(
            tokenCYON.allowance(msg.sender, address(this)) >= price,
            "CYON token allowance too low."
        ); // Make sure the Master contract has the allowance to manage the CYON transfer
        address currentOwner = NFTCollections(_collectionAddress).ownerOf(
            _tokenID
        );
        ...
        require(
            NFTCollections(_collectionAddress).getApproved(_tokenID) ==
                address(this),
            "This address does not have the allowance to send the NFT"
        ); // Make sure the Master contract has the allowance to manage the NFT transfer
        NFTCollections(_collectionAddress).safeTransferFrom(
            currentOwner,
            msg.sender,
            _tokenID
        );
        ...
        emit NFTSold(
            NFTCollections(_collectionAddress).name(),
            _collectionAddress,
            _tokenID,
            currentOwner,
            msg.sender,
    ...
    function requestAuction(address _collectionAddress, uint256 _tokenID)
        external
        returns (bool)
    {
        require(
        ...
        require(
            NFTCollections(_collectionAddress).getPrice(_tokenID) == 0,
            "This NFT is already for sale."
        ); // Make sure the NFT is not already for sale
    ...
    function closeAuction(address _collectionAddress, uint256 _tokenID)
        external
    {
        require(
            _collectionAddress != address(0),
        ...
        require(
            this.isInAuction(_collectionAddress, _tokenID),
            "This NFT did not start an auction."
        ); // Make sure an auction started
        require(
            this.checkAuctionTimeExpired(_collectionAddress, _tokenID),
            "The auction has not finished yet."
        ); // Make sure the auction is finished
        ...
        address highestBidder = this.getCurrentHighestBidder(
            _collectionAddress,
            _tokenID
        );
        require(
            highestBidder != address(0),
            "Error: the highest bidder is the zero address."
        ); // Make sure the address of the highest bidder is valid
    ...
    function getURI(address _collectionAddress, uint256 _tokenID)
        external
        view
