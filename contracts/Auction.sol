...
contract Auction {
    ...
    /// @notice This function allows to check the total pending refunds of a user, if any
    /// @dev Call this function from the front to know how much pending refunds a user has
    /// @param _nftCollectionAddress Collection address of the given NFT
    /// @param _nftTokenID Token ID of the given NFT
    /// @return Returns the total amount of pending refunds
    function getPendingRefunds(address _nftCollectionAddress, uint _nftTokenID) external view returns (uint256) {
        require(_nftCollectionAddress != address(0),"The collection address needs to be different from zero.");    // Make sure the address is different from zero
        return(bidManager[_nftCollectionAddress][_nftTokenID].pendingRefunds[msg.sender]);
    }
}